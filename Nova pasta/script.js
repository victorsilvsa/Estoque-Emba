     ...
     * Aceita: "2026-05-01", "2026-05-01T08:00:00.000Z", Date, null/undefined.
    ...
    /* ══════════════════════════════════════════════════════════════
    SETTINGS — EXPORTAR / IMPORTAR / LIMPAR
    ══════════════════════════════════════════════════════════════ */

    function exportData() {
        const blob = new Blob(
            [JSON.stringify({ threads, counts, days: daysMeta, exportedAt: new Date().toISOString() }, null, 2)],
            { type: 'application/json' }
        );
        const u = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = u;
        a.download = `tatui_fios_${today()}.json`;
        a.click();
    ...
    /**
     * importData — lê o JSON exportado e mescla com os dados locais,
     * sem duplicar fios nem contagens. Seguro para usar em qualquer dispositivo.
     */
async function importData(event) {
    ...
    try {
        ...
        toast('Arquivo invalido — nao e um JSON valido', 'err');
        return;
    }

    if (!Array.isArray(parsed.threads) || !Array.isArray(parsed.counts)) {
        toast('Arquivo nao reconhecido — estrutura invalida', 'err');
        return;
    }

    if (!parsed.threads.length && !parsed.counts.length) {
        toast('Backup vazio — nada a importar', 'err');
        return;
    }

    // ── 1. LIMPA TUDO ANTES DE IMPORTAR ──────────────────────────
    ...
    for (const c of parsed.counts) {
        if (!c.threadId || !c.date) continue;
        const rec = { ...c, date: toYMD(c.date) || c.date };
        delete rec.id; // deixa o autoIncrement gerar ID novo, sem colisão
        const newId = await dbPut('counts', rec);
        rec.id = newId;
        counts.push(rec);
    }

    // ── 4. INSERE DIAS FECHADOS ───────────────────────────────────
    const impDays = parsed.days || {};
    for (const [rawDate, meta] of Object.entries(impDays)) {
        const nd = toYMD(rawDate) || rawDate;
        if (!nd) continue;
        const rec = { ...meta, date: nd };
        await dbPut('days', rec);
        daysMeta[nd] = rec;
    }

    // ── 5. NORMALIZA E ATUALIZA UI ────────────────────────────────
    normalizeCounts();

    toast(`Importado: ${threads.length} fios, ${counts.length} contagens`);
    closeSheet();
    renderDash();
    renderStock();
}
    ...
    async function seedHistoricalData() {
        ...
        const historicalData = [
            ['2026-05-01', [4946, 3216, 364, 164, 442, 76, 131, 466, 464, 2396, 106, 994, 574, 6288, 1014, 0, 616, 594]],
            ['2026-05-02', [5884, 3160, 364, 164, 398, 890, 131, 466, 464, 2396, 106, 994, 574, 7072, 1014, 0, 616, 594]],
            ['2026-05-04', [3614, 2770, 364, 164, 376, 868, 131, 466, 464, 2396, 106, 994, 552, 6640, 1014, 0, 616, 594]],
            ['2026-05-05', [2935, 3183, 364, 164, 376, 824, 131, 444, 442, 2396, 106, 994, 508, 6368, 1014, 0, 616, 594]],
            ['2026-05-06', [4713, 2315, 364, 164, 376, 802, 131, 444, 442, 2396, 106, 994, 486, 7600, 1014, 0, 616, 594]],
            ['2026-05-07', [5043, 2023, 364, 142, 354, 780, 109, 444, 442, 2396, 106, 994, 464, 7880, 1014, 0, 616, 594]],
            ['2026-05-08', [5404, 2282, 364, 142, 332, 736, 109, 422, 418, 2396, 106, 994, 420, 8352, 1014, 0, 616, 594]],
            ['2026-05-09', [6365, 2077, 364, 120, 288, 714, 109, 400, 396, 2396, 106, 994, 398, 8896, 1014, 0, 616, 594]],
            ['2026-05-11', [5977, 2661, 364, 120, 244, 692, 109, 378, 374, 2396, 106, 994, 376, 9304, 1014, 0, 616, 594]],
            ['2026-05-12', [5508, 1758, 364, 120, 1270, 670, 109, 356, 352, 2396, 106, 994, 1376, 9632, 1014, 0, 616, 594]],
            ['2026-05-13', [6099, 1783, 364, 120, 1226, 648, 109, 356, 352, 2396, 106, 994, 1332, 10212, 1014, 0, 616, 594]],
            ['2026-05-14', [6307, 1883, 364, 120, 1182, 626, 109, 356, 352, 2396, 106, 994, 1288, 11424, 1014, 0, 616, 594]],
            ['2026-05-15', [6538, 2002, 364, 120, 1138, 604, 109, 356, 352, 2396, 106, 994, 1244, 12464, 1014, 0, 616, 594]],
            ['2026-05-16', [7812, 2324, 364, 120, 1094, 582, 109, 356, 352, 2396, 106, 994, 1200, 12164, 1014, 0, 616, 594]],
            ['2026-05-18', [7385, 3465, 364, 120, 1050, 538, 109, 356, 352, 2396, 106, 994, 1156, 11072, 1014, 0, 616, 594]],
            ['2026-05-19', [7364, 3738, 364, 120, 1028, 516, 109, 356, 352, 2396, 106, 994, 1134, 9984, 1014, 0, 616, 594]],
            ['2026-05-20', [10490, 3206, 364, 120, 1006, 494, 109, 356, 352, 2396, 106, 994, 1112, 9120, 1014, 0, 616, 594]],
            ['2026-05-21', [12470, 3755, 364, 120, 960, 314, 109, 356, 352, 2396, 106, 994, 1090, 8421, 1014, 0, 616, 594]],
            ['2026-05-22', [13174, 3466, 364, 120, 682, 314, 109, 356, 352, 2396, 106, 994, 1012, 7580, 1014, 0, 616, 594]],
        ];
    ...
    async function init() {
        ...
        // Calendário sempre abre no mês atual (hoje)
        const now = new Date();
        calYear = now.getFullYear();
        calMonth = now.getMonth();

        // UI inicial — seções de contagem
